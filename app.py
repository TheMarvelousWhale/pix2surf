...
@app.route('/')
def hello():
    return 'Microservice C: Pix2surf'

@app.route('/<filename>', methods=['GET'])
def run(filename):
    if filename != "": 
        demo.filename = filename
        print(f"Running pix2surf pipeline on image {filename}")
        demo.run()
